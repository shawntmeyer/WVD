...
try {
	...
	if (!$SkipAuth) {
		...
		$AzContext = $null
		try {
			$AzAuth = Connect-AzAccount -Environment AzureUSGovernment -ApplicationId $ConnectionAsset.ApplicationId -CertificateThumbprint $ConnectionAsset.CertificateThumbprint -TenantId $ConnectionAsset.TenantId -SubscriptionId $ConnectionAsset.SubscriptionId -ServicePrincipal
