...
function Update-WVDHostPool {
    ...
    $CurrentDateTime = Convert-UTCtoLocalTime -TimeDifference $TimeDifference
    # Get DeadlineTime
    ...
    foreach ($SessionHost in $SessionHosts) {
        ...
        elseif ($VMInstance.Tags.ImageVersion -eq $TargetImageVersion) {
            ...
            $null = New-AzTag -ResourceId $VMInstance.id -Tag  @{'MaintenanceMode' = $true } # Used e.g. by the scaling script to identify machines to ignore
        }        
